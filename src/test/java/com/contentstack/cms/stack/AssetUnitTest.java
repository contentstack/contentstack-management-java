...
class AssetUnitTest {
    ...
    @Test
    void testFolderException() {
        Stack stack = new Contentstack.Builder().build().stack("apiKey", "token");
        Folder theFolder = stack.asset().folder();
        theFolder.clearParams();
        theFolder.addParam("key", "value");
        theFolder.removeParam("key");
        theFolder.addHeader("key", "value");
        Assertions.assertThrows(IllegalAccessError.class, theFolder::fetch);
    }
}
